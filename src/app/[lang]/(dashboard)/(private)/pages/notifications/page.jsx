...
export default function NotificationsPage() {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      // 3. Help & Support (Chat) - UPDATED LOGIC
      if (Array.isArray(data.helpAndSupports)) {
        combined = [
          ...combined,
          ...data.helpAndSupports.map(item => {
            const chatbox = item.chatbox || []
            const lastMessage = chatbox.length > 0 ? chatbox[chatbox.length - 1] : null

            // Logic to determine sender:
            // If the message userId is NOT the current logged-in vendorId, it is from the Admin.
            const isFromAdmin = lastMessage && lastMessage.userId !== vendorId

            let displayTitle = 'Support Ticket Update'
                    ...
                    <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-start' }}>
                      <Box sx={{ flex: 1, minWidth: 0 }}>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, color: 'text.secondary' }}>
                          ...
                          <Typography variant='caption'>{formatDate(notification.time)}</Typography>
                        </Box>
                      </Box>
