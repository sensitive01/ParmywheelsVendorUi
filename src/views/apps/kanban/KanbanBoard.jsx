...
const ParkingChargesKanban = () => {
  ...
  // Section toggle states (Temporary, Full Day, Monthly)
  ...
  // Full day type toggle (24 hours vs Full day)
  const [fullDayTypeToggles, setFullDayTypeToggles] = useState({
    'Car-Full Day': false, // false = Full day, true = 24 hours
    'Bike-Full Day': false,
    'Others-Full Day': false
  });
  ...
  // Set initial toggle states based on fetched data
  useEffect(() => {
    if (!loading && Object.keys(charges).length > 0) {
      ...
      // For each charge, enable the relevant category and section
      Object.keys(charges).forEach(key => {
        const [category, label] = key.split('-');
        
        // Only enable if slots are available
        const slotProperty = categoryToSlotProperty[category];
        const hasAvailableSlots = slotProperty && availableSlots[slotProperty] > 0;
        ...
        if (hasAvailableSlots) {
          ...
          if (label === 'Minimum Charges' || label === 'Additional Hour') {
            ...
            // Set full day type toggle based on charge type
            if (charges[key].type.includes('24')) {
              newFullDayTypeToggles[`${category}-Full Day`] = true; // 24 hours
            }
  ...
  const handleCategoryToggle = (category) => {
    // Only allow toggling if slots are available
    const slotProperty = categoryToSlotProperty[category];
    const hasAvailableSlots = slotProperty && availableSlots[slotProperty] > 0;
  ...
  const ChargeCard = ({ category, label }) => {
    ...
    // For Full Day charges, determine which type options to show based on toggle
  ...
  const CategorySection = ({ category }) => {
    ...
    return (
      <Box sx={{ mb: 4 }}>
        ...
        {/* If category is enabled, show section toggles */}
        {isCategoryEnabled && (
          <Box sx={{ pl: 3, pt: 1 }}>
            {/* Temporary Section Toggle */}
            <SectionToggle>
              <Box sx={{ display: 'flex', alignItems: 'center' }}>
                <ScheduleIcon />
            ...
            {/* If Temporary is enabled, show minimum charges and additional hour */}
            ...
            {/* If Full Day is enabled, show Full Day options */}
            ...
            {/* If Monthly is enabled, show Monthly options */}
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 4 } }}>
      ...
      {/* Display categories with toggle switches */}
      <Box sx={{ width: '100%' }}>
        {categories.map((category) => (
