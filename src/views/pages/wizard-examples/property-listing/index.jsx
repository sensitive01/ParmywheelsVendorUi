...
export default function ParkingBooking() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Add to notification store for history and components
  ...
  const renderVehicleTypeStep = () => (
    <Box>
      ...
      <Grid container spacing={2}>
        {[
          { value: 'Cars', label: 'Cars', icon: DirectionsCarFilled },
          { value: 'Bikes', label: 'Bikes', icon: TwoWheeler },
          { value: 'Other', label: 'Other', icon: LocalShipping }
        ].map((option) => (
